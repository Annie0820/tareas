...
public class AVLTree <T> {
    private AVLNode<T> no;

    public void insert( Comparable<T> x ){
        no = insert( x, no );
    }

    private AVLNode<T> insert( Comparable x, AVLNode<T> t ){
        if( t == null )
    ...
    private  static <T>int max( int lhs, int rhs ){
        return lhs > rhs ? lhs : rhs;
    }


    private static <T> AVLNode rotateWithLeftChild( AVLNode<T> k2 ){
    ...
    private static <T>AVLNode rotateWithRightChild( AVLNode k1 ){
    ...
    private static <T>AVLNode doubleWithLeftChild( AVLNode k3 ){
        k3.izquierdo = rotateWithRightChild( k3.izquierdo );
        return rotateWithLeftChild( k3 );
    }


    private static <T>AVLNode doubleWithRightChild( AVLNode k1 ){
    ...
    private void imprimirXaltura(AVLNode<T> nodo){
        if ( nodo != null ){
            ...
            System.out.println( replicate(" ",height(no) - height(nodo)) +"["+ nodo.dato + "]");
